...
public class PlingSyntaxHighlighter extends SyntaxHighlighterBase {
    ...
    public Lexer getHighlightingLexer() {
        return new PlingLexerAdapter();
    }

    @Override
    public TextAttributesKey @NotNull [] getTokenHighlights(IElementType tokenType) {
        if (tokenType.equals(PlingTypes.SEPARATOR)) {
            return SEPARATOR_KEYS;
        }
        if (tokenType.equals(PlingTypes.KEY)) {
            return KEY_KEYS;
        }
        if (tokenType.equals(PlingTypes.VALUE)) {
            return VALUE_KEYS;
        }
        if (tokenType.equals(PlingTypes.COMMENT)) {
            return COMMENT_KEYS;
        }
